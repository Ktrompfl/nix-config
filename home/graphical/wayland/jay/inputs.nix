{ helpers, ... }:
let
  inherit (helpers.jay) disableConnector enableConnector;
in
{
  xdg.config.files."jay/config.toml".value = {
    ...
    inputs = [
      ...
      }
      {
        ...
        on-lid-closed = disableConnector { name = "laptop-integrated"; };
        on-lid-opened = enableConnector { name = "laptop-integrated"; };
      }
    ];
  };
}
