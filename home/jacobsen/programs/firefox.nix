...
let
  ...
  # userChrome.css shared by every profile
  commonUserChrome = /* css */ ''
  ...
  # extensions: containers share a single profile, so extensions can't be
  # scoped per-container (only cookies/storage/identity are isolated) -
  # everything is installed for all containers
  ...
  # search engines: same reasoning as extensions - one profile means one
  # shared set of engines across all containers
  commonSearchEngines = {
    ...
    firefox = {
      ...
      profiles = {
        default = {
          ...
          containers = {
            # personal, as opposed to develop/science - not incognito/private browsing
            private = {
              id = 1;
              color = "blue";
              icon = "fingerprint";
