...
let
  # Everything the parts below share. This is the counterpart of
  # ../../../../pkgs/jay-config-lib/src/lib.rs.
  jayLib = rec {
    # --- action constructors ---
    #
  ...
  # The sixteen base16 slots of the active stylix scheme, lower-cased because
  # that is how the shared library configuration reads them back.
  base16 = lib.mapAttrs' (name: lib.nameValuePair (lib.toLower name)) (
    lib.filterAttrs (name: _: builtins.match "base0[0-9A-F]" name != null) config.lib.stylix.colors
  );
in
{
  ...
  wayland.windowManager.jay = {
    ...
    settings = lib.mergeAttrsList settings;
  };
