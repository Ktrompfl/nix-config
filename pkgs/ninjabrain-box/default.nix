...
craneLib.buildPackage (
  ...
  // {
    ...
    postInstall = ''
      ...
      # Generated by the binary itself, from the same action table the overlay
      # dispatches on, so the completions can never list an action that is not
      # there. Safe to run here: `completions` reaches neither the compositor
      # nor the bot.
      installShellCompletion --cmd ninjabrain-box \
        --bash <($out/bin/ninjabrain-box completions bash) \
        --fish <($out/bin/ninjabrain-box completions fish) \
        --zsh <($out/bin/ninjabrain-box completions zsh)
